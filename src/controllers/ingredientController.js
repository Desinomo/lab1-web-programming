...
const getIngredientById = async (req, res, next) => {
    try {
        ...
        console.log(`Serving ingredient ${ingredientId} from DATABASE`);
...
const createIngredient = async (req, res, next) => {
    try {
        ...
        if (!name || name.trim().length < 2)
            return res.status(400).json({ error: "Ingredient name must be at least 2 characters" });
        ...
        io.to('ADMIN').to('MODERATOR').emit('notification:new', {
            type: 'success',
            message: `New ingredient added: ${ingredient.name}`,
...
const updateIngredient = async (req, res, next) => {
    try {
        ...
        io.to('ADMIN').to('MODERATOR').emit('notification:new', {
            type: 'info',
            message: `Ingredient updated: ${ingredient.name}`,
            ingredientId: ingredient.id
        });
...
const deleteIngredient = async (req, res, next) => {
    try {
        ...
        if (recipesCount > 0)
            return res.status(400).json({ error: `Cannot delete ingredient. Used in ${recipesCount} recipe(s).` });
        ...
        if (err.code === 'P2025')
            return res.status(404).json({ error: "Ingredient not found for deletion" });
        next(err);
    }
};

// ✅ ВАЖЛИВО: Закриваємо module.exports ПРАВИЛЬНО
module.exports = {
    getAllIngredients,
    getIngredientById,
    createIngredient,
